	...
	return (
		<div className="space-y-8 p-6">
			...
			<BentoGrid className="gap-4">
				{/* Welcome Card */}
				<BentoWelcomeCard subtitle="Here's what's happening with your infrastructure today." />

				{/* Status Stats */}
				{summary.isLoading ? (
					<>
						<BentoItem className="h-32">
					...
					<div className="flex items-center justify-between mb-6">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-xs text-muted-foreground">
									Live health checks for all platform services.
								</p>
							</div>
						</div>
						...
						{downChecks.length > 0 && (
							<Badge variant="destructive" className="animate-pulse">
								{downChecks.length} Service{downChecks.length > 1 ? "s" : ""}{" "}
								Reporting Issues
							</Badge>
						)}
					</div>
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
						{[
							...
							},
							{
								id: "yaade",
								name: "API Testing",
								path: `${SKYFORGE_API}/yaade/sso`,
								icon: Zap,
								enabled: features?.yaadeEnabled ?? false,
								external: true,
							},
							{
								id: "webhooks",
							...
							.map((tool) => {
								...
								const check = checks.find((c) =>
									c.name.toLowerCase().includes(tool.id),
								);
								const isUp = check
								...
								return (
									...
									>
										<div
											className={`absolute top-2 right-2 w-2 h-2 rounded-full ${statusColor} ${!isUp ? "animate-pulse" : ""}`}
										/>
							...
							<div className="flex flex-wrap gap-2">
								{downChecks.map((c) => (
									<Badge
										key={c.name}
										variant="outline"
										className="text-[10px] border-red-500/30 text-red-400 bg-red-500/5"
									>
										{c.name}: {c.status}
									</Badge>
								))}
