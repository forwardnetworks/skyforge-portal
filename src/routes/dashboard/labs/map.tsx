...
import { getWorkspaceContainerlabTemplate, getWorkspaces, type DeploymentTopology } from "@/lib/skyforge-api";
...
function LabsMapPage() {
  ...
  const templateQ = useQuery({
    queryKey: workspaceId ? ["containerlabTemplateMap", workspaceId, source, dir, file] : ["containerlabTemplateMap", "none"],
    queryFn: async () => {
      if (!workspaceId) throw new Error("workspaceId is required");
      if (!file) throw new Error("file is required");
      return getWorkspaceContainerlabTemplate(workspaceId, { source, dir, file });
    },
    ...
    return (
      <div className="h-screen w-screen p-6">
        <div className="max-w-2xl space-y-4">
          ...
          <div className="text-sm text-muted-foreground">Open this page from the Lab Designer “Open map” button, or provide query params.</div>
          <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-1">
                ...
                <Input value={workspaceId} readOnly placeholder={workspacesQ.isLoading ? "Loading…" : "workspaceId missing"} />
              </div>
              <div className="space-y-1">
                <Label>File</Label>
              ...
              <div className="text-xs text-muted-foreground">
                Example: <span className="font-mono">/dashboard/labs/map?workspaceId=&lt;id&gt;&amp;source=workspace&amp;dir=containerlab/designer&amp;file=lab.clab.yml</span>
              </div>
            </CardContent>
          </Card>
        </div>
    ...
    return (
      <div className="h-screen w-screen p-6">
        <EmptyState
          ...
          description={templateQ.isError ? "Failed to load template." : "Template YAML could not be parsed into a topology."}
      ...
      <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground font-mono truncate">{templateQ.data.path}</div>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[1fr_520px]">
        <div className="min-h-0 p-3">
          <div className="h-full rounded-xl border overflow-hidden">
            <TopologyViewer topology={topology as any} workspaceId={workspaceId} enableTerminal={false} fullHeight />
          </div>
        </div>
        <div className="min-h-0 border-l bg-background p-3">
          <Card className="h-full">
            ...
            <CardContent className="h-[calc(100%-56px)]">
              <Textarea value={templateQ.data.yaml} readOnly className="h-full font-mono text-xs" />
            </CardContent>
          </Card>
        </div>
      </div>
