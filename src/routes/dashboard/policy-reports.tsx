...
import {
	...
	approveUserPolicyReportException,
	attestUserPolicyReportRecertAssignment,
	createUserPolicyReportCustomRun,
	createUserPolicyReportException,
	createUserPolicyReportForwardNetwork,
	createUserPolicyReportRecertCampaign,
	createUserPolicyReportRun,
	createUserPolicyReportZone,
	deleteUserPolicyReportForwardNetwork,
	deleteUserPolicyReportZone,
	generateUserPolicyReportRecertAssignments,
	getUserPolicyReportCheck,
	getUserPolicyReportChecks,
	getUserPolicyReportPacks,
	getUserPolicyReportRun,
	getUserPolicyReportSnapshots,
	listUserForwardCredentialProfiles,
	listUserForwardNetworksFromSource,
	listUserPolicyReportExceptions,
	listUserPolicyReportFindings,
	listUserPolicyReportForwardNetworks,
	listUserPolicyReportRecertAssignments,
	listUserPolicyReportRecertCampaigns,
	listUserPolicyReportRunFindings,
	listUserPolicyReportRuns,
	listUserPolicyReportZones,
	rejectUserPolicyReportException,
	runUserPolicyReportCheck,
	runUserPolicyReportPack,
	runUserPolicyReportPackDelta,
	simulateUserPolicyReportChangePlanning,
	waiveUserPolicyReportRecertAssignment,
...
function PolicyReportsPage() {
	const LATEST_SNAPSHOT = "__latest__";
	const ALL_DEVICES = "__all__";
	const userScopeId = "self";
	...
	const checks = useQuery({
		queryKey: queryKeys.policyReportsChecks(),
		queryFn: () => getUserPolicyReportChecks(),
		enabled: true,
		staleTime: 60_000,
	});

	const packs = useQuery({
		queryKey: queryKeys.policyReportsPacks(),
		queryFn: () => getUserPolicyReportPacks(),
		enabled: true,
		staleTime: 60_000,
	});

	const forwardNetworks = useQuery({
		queryKey: ["policyReportsForwardNetworks", userScopeId],
		queryFn: () => listUserPolicyReportForwardNetworks(),
		enabled: true,
		staleTime: 10_000,
	});

	const forwardProfiles = useQuery({
	...
	const sourceNetworks = useQuery({
		queryKey: queryKeys.userForwardSourceNetworks(
		...
		queryFn: () =>
			listUserForwardNetworksFromSource({
				forwardCredentialProfileId:
					selectedForwardCredentialProfileId.trim() || undefined,
				limit: 100,
			}),
		enabled: selectedForwardCredentialProfileId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	});

	const campaigns = useQuery({
		queryKey: queryKeys.policyReportsRecertCampaigns(),
		queryFn: () => listUserPolicyReportRecertCampaigns(userScopeId),
		enabled: true,
		staleTime: 10_000,
	});
	...
	const assignments = useQuery({
		...
		enabled: true && !!selectedCampaignId,
		staleTime: 10_000,
	});

	const exceptions = useQuery({
		queryKey: queryKeys.policyReportsExceptions(networkId.trim()),
		queryFn: () =>
			listUserPolicyReportExceptions(
		...
		enabled: true && networkId.trim().length > 0,
		staleTime: 10_000,
	});

	const snapshots = useQuery({
		queryKey: queryKeys.policyReportsSnapshots(networkId.trim()),
		queryFn: async () => {
			return getUserPolicyReportSnapshots(networkId.trim(), 25);
		},
		enabled: true && networkId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	...
	const zones = useQuery({
		queryKey: queryKeys.policyReportsZones(networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportZones(net);
		},
		enabled: true && networkId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	...
	const runs = useQuery({
		queryKey: queryKeys.policyReportsRuns(networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportRuns(net, undefined, undefined, 50);
		},
		enabled: true && networkId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	...
	const runDetail = useQuery({
		queryKey: queryKeys.policyReportsRun(runDialogId),
		queryFn: () => getUserPolicyReportRun(runDialogId),
		enabled: true && runDialogOpen && runDialogId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	});
	...
	const runFindings = useQuery({
		...
		queryFn: async () => {
			...
			return listUserPolicyReportRunFindings(
				runId,
				runDialogCheckId.trim() || undefined,
				500,
			);
	...
	const aggFindings = useQuery({
		queryKey: queryKeys.policyReportsFindings(networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportFindings(net, undefined, "ACTIVE", 500);
		},
		enabled: true && networkId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	...
	const openCheck = useMutation({
		mutationFn: async (id: string) => {
			return getUserPolicyReportCheck(id);
		},
		onError: (e) =>
	...
	const runCheck = useMutation({
		mutationFn: async (checkId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return runUserPolicyReportCheck({
				networkId: net,
	...
	const runIntentSuite = useMutation({
		mutationFn: async () => {
			...
			for (const f of flows) {
				const resp = await runUserPolicyReportCheck({
					networkId: net,
					snapshotId: snapshotId.trim() || undefined,
					checkId: "acl-flow-decision.nqe",
					parameters: {
	...
	const storeIntentSuiteAsRuns = useMutation({
		mutationFn: async () => {
			...
			for (const f of flows) {
				...
				const resp = await createUserPolicyReportCustomRun({
	...
	const runPack = useMutation({
		mutationFn: async (packId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return runUserPolicyReportPack({
				networkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId,
			});
	...
	const runPackAndStore = useMutation({
		mutationFn: async (packId: string) => {
			...
			return createUserPolicyReportRun({
				forwardNetworkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId: packId.trim(),
			});
	...
	const runDelta = useMutation({
		mutationFn: async () => {
			...
			return runUserPolicyReportPackDelta({
				networkId: net,
				packId,
	...
	const createForwardNetwork = useMutation({
		mutationFn: async () => {
			...
			if (!forwardCredentialProfileId) {
				throw new Error("Forward credential profile is required");
			}
	...
	const deleteForwardNetwork = useMutation({
		mutationFn: async (networkRef: string) => {
			...
			return deleteUserPolicyReportForwardNetwork(networkRef.trim());
		},
		onSuccess: async () => {
	...
	const createZone = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportZone(net, {
				name,
	...
	const deleteZone = useMutation({
		mutationFn: async (zoneId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return deleteUserPolicyReportZone(net, zoneId);
		},
		onSuccess: async () => {
	...
	const storeSegmentationReport = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportCustomRun({
				forwardNetworkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId: "segmentation",
				title,
	...
	const createCampaign = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportRecertCampaign({
				name,
	...
	const generateAssignments = useMutation({
		mutationFn: async () => {
			...
			return generateUserPolicyReportRecertAssignments(selectedCampaignId, {
	...
	const attestAssignment = useMutation({
		mutationFn: async (req: {
			...
			return attestUserPolicyReportRecertAssignment(req.assignmentId, {
				justification,
			});
		},
		onSuccess: async () => {
	...
	const waiveAssignment = useMutation({
		mutationFn: async (req: {
			...
			const justification = req.justification.trim();
			if (!justification) throw new Error("Justification is required");
			return waiveUserPolicyReportRecertAssignment(req.assignmentId, {
				justification,
			});
		},
		onSuccess: async () => {
	...
	const createException = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportException({
				forwardNetworkId,
				findingId,
				checkId,
				justification,
	...
	const approveException = useMutation({
		mutationFn: async (exceptionId: string) => {
			return approveUserPolicyReportException(exceptionId);
		},
		onSuccess: async () => {
	...
	const rejectException = useMutation({
		mutationFn: async (exceptionId: string) => {
			return rejectUserPolicyReportException(exceptionId);
		},
		onSuccess: async () => {
	...
	const runChangePlanning = useMutation({
		mutationFn: async () => {
			...
			return simulateUserPolicyReportChangePlanning({
				networkId: net,
				snapshotId: snapshotId.trim() || undefined,
	...
	const forwardNetworksList: PolicyReportForwardNetwork[] = useMemo(
		() =>
			(forwardNetworks.data?.networks ?? []).filter(
				(n) =>
					typeof n?.id === "string" &&
					n.id.trim().length > 0 &&
					typeof n?.forwardNetworkId === "string" &&
					n.forwardNetworkId.trim().length > 0,
			),
		[forwardNetworks.data?.networks],
	);
	const forwardProfilesList = useMemo(
		() =>
			(forwardProfiles.data?.profiles ?? []).filter(
				(p) =>
					typeof p?.id === "string" &&
					p.id.trim().length > 0 &&
		...
		[forwardProfiles.data?.profiles],
	);
	const sourceNetworksList = useMemo(
		() =>
			(sourceNetworks.data?.networks ?? []).filter(
				(n) =>
					typeof n?.id === "string" &&
					n.id.trim().length > 0 &&
				...
				<div className="flex items-center gap-2">
					...
					<Button
						...
						onClick={() => {
							const net = networkId.trim();
							downloadJSON(
								`network-analytics_${userScopeId}_${net || "network"}_${snapshotId.trim() || "latest"}.json`,
								{
									userScopeId,
									networkId: net,
									snapshotId: snapshotId.trim() || undefined,
...
<body>
  ...
  <div class="meta">
    <span class="chip">userScope=<span class="mono">${esc(userScopeId)}</span></span>
  ...
  ${bodyHTML}
							...
							downloadText(
								`network-analytics_${userScopeId}_${net || "network"}_${snapshotId.trim() || "latest"}.html`,
								"text/html",
								html,
							);
						}}
				...
				<CardContent className="grid gap-4 md:grid-cols-3">
					...
					<div className="space-y-2">
						<div className="text-sm font-medium">
							Forward credential profile
						</div>
						<Select
							value={selectedForwardCredentialProfileId}
							onValueChange={setSelectedForwardCredentialProfileId}
						>
							<SelectTrigger>
								<SelectValue placeholder="Select My Settings credential" />
							</SelectTrigger>
							<SelectContent>
								{forwardProfilesList.map((p) => (
						...
						{forwardProfiles.isError ? (
							<p className="text-xs text-destructive">
								Failed to load profiles:{" "}
								{(forwardProfiles.error as Error).message}
							</p>
						) : null}
						...
						{sourceNetworks.isError ? (
							<p className="text-xs text-destructive">
								Forward network lookup failed:{" "}
								{(sourceNetworks.error as Error).message}
							</p>
						) : null}
					...
					<div className="space-y-2">
						...
						<Select
							value={snapshotId || LATEST_SNAPSHOT}
							onValueChange={(v) =>
								setSnapshotId(v === LATEST_SNAPSHOT ? "" : v)
							}
						>
							<SelectTrigger>
										...
										<div className="space-y-3">
											<div className="grid gap-3 md:grid-cols-2">
												<div className="space-y-2">
													...
													<Select
														value={flowSelectedDevice || ALL_DEVICES}
														onValueChange={(v) =>
															setFlowSelectedDevice(v === ALL_DEVICES ? "" : v)
														}
													>
														<SelectTrigger>
														...
														<SelectContent>
															<SelectItem value={ALL_DEVICES}>(All)</SelectItem>
															{devices.map((d) => (
