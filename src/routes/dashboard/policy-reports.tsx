...
import {
	...
	deleteUserPolicyReportForwardNetworkCredentials,
	deleteUserPolicyReportPreset,
	deleteUserPolicyReportZone,
	generateUserPolicyReportRecertAssignments,
	getUserPolicyReportCheck,
	getUserPolicyReportChecks,
	getUserPolicyReportForwardNetworkCredentials,
	...
	getUserContexts,
	putUserPolicyReportForwardNetworkCredentials,
	rejectUserPolicyReportException,
	runUserPolicyReportCheck,
	runUserPolicyReportPack,
	runUserPolicyReportPackDelta,
	runUserPolicyReportPathsEnforcementBypass,
	runUserPolicyReportPreset,
...
function PolicyReportsPage() {
	...
	const checks = useQuery({
		queryKey: queryKeys.policyReportsChecks(userContextId),
		queryFn: () => getUserPolicyReportChecks(userContextId),
		staleTime: 60_000,
	});

	const packs = useQuery({
		queryKey: queryKeys.policyReportsPacks(userContextId),
		queryFn: () => getUserPolicyReportPacks(userContextId),
		staleTime: 60_000,
	});

	const forwardNetworks = useQuery({
		queryKey: ["policyReportsForwardNetworks", userContextId],
		queryFn: () => listUserPolicyReportForwardNetworks(userContextId),
		staleTime: 10_000,
	});

	const campaigns = useQuery({
		queryKey: queryKeys.policyReportsRecertCampaigns(userContextId),
		queryFn: () => listUserPolicyReportRecertCampaigns(userContextId),
		staleTime: 10_000,
	});
	...
	const exceptions = useQuery({
		queryKey: queryKeys.policyReportsExceptions(
			userContextId,
			networkId.trim(),
		),
		queryFn: () =>
	...
	const snapshots = useQuery({
		queryKey: queryKeys.policyReportsSnapshots(userContextId, networkId.trim()),
		queryFn: async () => {
			return getUserPolicyReportSnapshots(
				userContextId,
				networkId.trim(),
				25,
			);
	...
	const forwardCreds = useQuery({
		queryKey: ["policyReportsForwardCreds", userContextId, networkId.trim()],
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return getUserPolicyReportForwardNetworkCredentials(
				userContextId,
				net,
			);
		},
	...
	const zones = useQuery({
		queryKey: queryKeys.policyReportsZones(userContextId, networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportZones(userContextId, net);
		},
		enabled: networkId.trim().length > 0,
		retry: false,
		staleTime: 10_000,
	...
	const presets = useQuery({
		queryKey: ["policyReportsPresets", userContextId, networkId.trim()],
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportPresets(
				userContextId,
				net,
				undefined,
				200,
	...
	const runs = useQuery({
		queryKey: queryKeys.policyReportsRuns(userContextId, networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportRuns(
				userContextId,
				net,
				undefined,
				undefined,
	...
	const runDetail = useQuery({
		queryKey: queryKeys.policyReportsRun(userContextId, runDialogId),
		queryFn: () => getUserPolicyReportRun(userContextId, runDialogId),
	...
	const aggFindings = useQuery({
		queryKey: queryKeys.policyReportsFindings(userContextId, networkId.trim()),
		queryFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return listUserPolicyReportFindings(
				userContextId,
				net,
				undefined,
				"ACTIVE",
	...
	const openCheck = useMutation({
		mutationFn: async (id: string) => {
			return getUserPolicyReportCheck(userContextId, id);
		},
		onError: (e) =>
	...
	const runCheck = useMutation({
		mutationFn: async (checkId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return runUserPolicyReportCheck(userContextId, {
				networkId: net,
	...
	const runIntentSuite = useMutation({
		mutationFn: async () => {
			...
			for (const f of flows) {
				const resp = await runUserPolicyReportCheck(userContextId, {
					networkId: net,
					snapshotId: snapshotId.trim() || undefined,
					checkId: "acl-flow-decision.nqe",
					parameters: {
	...
	const storeIntentSuiteAsRuns = useMutation({
		mutationFn: async () => {
			...
			for (const f of flows) {
				...
				const resp = await createUserPolicyReportCustomRun(userContextId, {
	...
	const runPathsBypassSuite = useMutation({
		mutationFn: async () => {
			...
			return runUserPolicyReportPathsEnforcementBypass(userContextId, {
				forwardNetworkId: net,
				snapshotId: snapshotId.trim() || undefined,
				queries,
	...
	const runPack = useMutation({
		mutationFn: async (packId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return runUserPolicyReportPack(userContextId, {
				networkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId,
			});
	...
	const runPackAndStore = useMutation({
		mutationFn: async (packId: string) => {
			...
			return createUserPolicyReportRun(userContextId, {
				forwardNetworkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId: packId.trim(),
			});
	...
	const runDelta = useMutation({
		mutationFn: async () => {
			...
			return runUserPolicyReportPackDelta(userContextId, {
				networkId: net,
				packId,
	...
	const createForwardNetwork = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportForwardNetwork(userContextId, {
				forwardNetworkId,
				name,
	...
	const deleteForwardNetwork = useMutation({
		mutationFn: async (networkRef: string) => {
			...
			return deleteUserPolicyReportForwardNetwork(
				userContextId,
				networkRef.trim(),
			);
		},
	...
	const putForwardCreds = useMutation({
		mutationFn: async () => {
			...
			if (credsCredentialSetMode !== "custom") {
				...
				return putUserPolicyReportForwardNetworkCredentials(
					userContextId,
					net,
					{
	...
	const deleteForwardCreds = useMutation({
		mutationFn: async () => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return deleteUserPolicyReportForwardNetworkCredentials(
				userContextId,
				net,
			);
		},
	...
	const createZone = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportZone(userContextId, net, {
				name,
	...
	const deleteZone = useMutation({
		mutationFn: async (zoneId: string) => {
			const net = networkId.trim();
			if (!net) throw new Error("Network ID is required");
			return deleteUserPolicyReportZone(userContextId, net, zoneId);
		},
		onSuccess: async () => {
	...
	const storeSegmentationReport = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportCustomRun(userContextId, {
				forwardNetworkId: net,
				snapshotId: snapshotId.trim() || undefined,
				packId: "segmentation",
				title,
	...
	const createPreset = useMutation({
		mutationFn: async () => {
			...
			if (presetKind === "PACK") {
				...
				if (!packId) throw new Error("Pack is required");
				return createUserPolicyReportPreset(userContextId, {
					forwardNetworkId: net,
					name,
					kind: "PACK",
					packId,
	...
	const deletePreset = useMutation({
		mutationFn: async (presetId: string) => {
			return deleteUserPolicyReportPreset(userContextId, presetId);
		},
		onSuccess: async () => {
	...
	const runPresetNow = useMutation({
		mutationFn: async (presetId: string) => {
			return runUserPolicyReportPreset(userContextId, presetId);
		},
		onSuccess: async (resp) => {
	...
	const createCampaign = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportRecertCampaign(userContextId, {
				name,
	...
	const generateAssignments = useMutation({
		mutationFn: async () => {
			...
			return generateUserPolicyReportRecertAssignments(
				userContextId,
				selectedCampaignId,
				{
	...
	const attestAssignment = useMutation({
		mutationFn: async (req: {
			...
			return attestUserPolicyReportRecertAssignment(
				userContextId,
				req.assignmentId,
				{ justification },
			);
	...
	const waiveAssignment = useMutation({
		mutationFn: async (req: {
			...
			const justification = req.justification.trim();
			if (!justification) throw new Error("Justification is required");
			return waiveUserPolicyReportRecertAssignment(
				userContextId,
				req.assignmentId,
				{ justification },
			);
	...
	const createException = useMutation({
		mutationFn: async () => {
			...
			return createUserPolicyReportException(userContextId, {
				forwardNetworkId,
				findingId,
				checkId,
				justification,
	...
	const approveException = useMutation({
		mutationFn: async (exceptionId: string) => {
			return approveUserPolicyReportException(userContextId, exceptionId);
		},
		onSuccess: async () => {
	...
	const rejectException = useMutation({
		mutationFn: async (exceptionId: string) => {
			return rejectUserPolicyReportException(userContextId, exceptionId);
		},
		onSuccess: async () => {
	...
	const runChangePlanning = useMutation({
		mutationFn: async () => {
			...
			return simulateUserPolicyReportChangePlanning(userContextId, {
				networkId: net,
				snapshotId: snapshotId.trim() || undefined,
					...
					<div className="flex items-center gap-2">
						...
						<Button
							...
							onClick={() => {
								const net = networkId.trim();
								downloadJSON(
									`policy-reports_user-${userContextId}_${net || "network"}_${snapshotId.trim() || "latest"}.json`,
									{
										userContextId,
										networkId: net,
										snapshotId: snapshotId.trim() || undefined,
...
<body>
  ...
  <div class="meta">
    <span class="chip">userContext=<span class="mono">${esc(userContextId)}</span></span>
  ...
  ${bodyHTML}
								...
								downloadText(
									`policy-reports_user-${userContextId}_${net || "network"}_${snapshotId.trim() || "latest"}.html`,
									"text/html",
									html,
								);
							}}
				...
				<CardContent className="grid gap-4 md:grid-cols-3">
					...
					<div className="space-y-2">
						...
						<p className="text-xs text-muted-foreground">
							...
							the saved user Forward credentials on the server.
						</p>
					</div>
					<div className="space-y-2">
