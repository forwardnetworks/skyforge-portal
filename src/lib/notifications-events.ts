...
export function useNotificationsEvents(enabled: boolean, includeRead: boolean, limit: string) {
  ...
  useEffect(() => {
    ...
    const onSnapshot = (ev: MessageEvent<string>) => {
      try {
        ...
        queryClient.setQueryData(queryKeys.notifications(includeRead, limit), payload);
      } catch {
        // ignore parse errors
      }
    };
