...
import { SKYFORGE_API, type DeploymentUIEvent } from "./skyforge-api";
...
export function useDeploymentUIEvents(workspaceId: string, deploymentId: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const onUIEvents = (ev: MessageEvent<string>) => {
      try {
        ...
        queryClient.setQueryData(queryKeys.deploymentUIEvents(workspaceId, deploymentId), (prev?: DeploymentUIEventsState) => {
