...
export const NETLAB_ENV_KEYS: EnvKeyOption[] = [
  ...
  { label: "Netlab `--set` overrides (`SKYFORGE_NETLAB_SET_OVERRIDES`)", key: "SKYFORGE_NETLAB_SET_OVERRIDES" },
];
...
export function isNetlabMultilineKey(keyRaw: string): boolean {
  return String(keyRaw ?? "").trim().toUpperCase() === "SKYFORGE_NETLAB_SET_OVERRIDES";
}
