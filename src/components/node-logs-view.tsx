...
export function NodeLogsView({
	...
	const logs = useQuery({
		queryKey: ["deploymentNodeLogs", userContextId, deploymentId, nodeId, tail],
		enabled: !!userContextId && !!deploymentId && !!nodeId,
		queryFn: async () =>
			getDeploymentNodeLogs(userContextId, deploymentId, nodeId, {
				tail,
				container: nodeId,
			}),
			...
			<div className="flex items-center justify-between gap-2 p-2 border-b bg-muted/40 text-xs">
				...
				<div className="flex items-center gap-2">
					...
					>
						<RefreshCw
							className={
								logs.isFetching ? "mr-2 h-3 w-3 animate-spin" : "mr-2 h-3 w-3"
							}
						/>
						Refresh
					</Button>
