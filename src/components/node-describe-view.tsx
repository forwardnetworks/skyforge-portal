...
export function NodeDescribeView({ workspaceId, deploymentId, nodeId, nodeKind, nodeIp, onClose, className }: Props) {
  const title = useMemo(() => {
    const parts = [nodeId];
    ...
    <div className={`flex flex-col h-full w-full ${className ?? ""}`}>
      <div className="flex items-center justify-between gap-2 p-2 border-b bg-muted/40 text-xs">
        ...
        <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="h-7" onClick={() => data.refetch()} disabled={data.isFetching}>
              <RefreshCw className={data.isFetching ? "mr-2 h-3 w-3 animate-spin" : "mr-2 h-3 w-3"} />
              Refresh
            </Button>
            {onClose ? (
