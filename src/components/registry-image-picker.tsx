...
export function RegistryImagePicker(props: {
  ...
  const tagsQ = useQuery({
    queryKey: repo ? queryKeys.registryTags(repo, tagQuery) : ["registryTags", "none"],
  ...
  return (
    <div className={`space-y-3 ${props.className ?? ""}`}>
      <div className="space-y-1">
        <Label>Image</Label>
        <Input value={props.value} onChange={(e) => props.onChange(e.target.value)} placeholder="repo:tag" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                toast.message("Repository selected", { description: selected.repo });
              }}
            >
              Use current
            </Button>
          </div>
          <Input value={repoQuery} onChange={(e) => setRepoQuery(e.target.value)} placeholder="Search repos…" />
          <div className="max-h-40 overflow-auto rounded-md border">
            {reposQ.isError ? (
              <div className="p-2 text-xs text-destructive">Failed to load repos.</div>
            ) : reposQ.isLoading ? (
        ...
        <div className="space-y-2">
          ...
          <Input value={tagQuery} onChange={(e) => setTagQuery(e.target.value)} placeholder="Search tags…" disabled={!repo} />
          <div className="max-h-40 overflow-auto rounded-md border">
            {!repo ? (
              <div className="p-2 text-xs text-muted-foreground">Select a repo…</div>
            ) : tagsQ.isError ? (
              <div className="p-2 text-xs text-destructive">Failed to load tags.</div>
            ) : tagsQ.isLoading ? (
              <div className="p-2 text-xs text-muted-foreground">Loading…</div>
            ) : (
              <div className="divide-y">
