...
export function WorkspaceVariableGroups({ workspaceId, allowEdit }: Props) {
	...
	const [editingGroup, setEditingGroup] =
		useState<WorkspaceVariableGroup | null>(null);
	...
	const deleteMutation = useMutation({
		mutationFn: async (groupId: number) =>
			deleteWorkspaceVariableGroup(workspaceId, groupId),
		onSuccess: () => {
			...
			<CardHeader>
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>
							Define sets of environment variables that can be injected into
							deployments.
						</CardDescription>
					</div>
					<Button
	...
	const mutation = useMutation({
		mutationFn: async () => {
			...
			if (mode === "create") {
				...
				return updateWorkspaceVariableGroup(
					workspaceId,
					initialData.id,
					payload,
				);
			...
			<DialogContent className="max-w-2xl">
									...
									>
										<div className="col-span-5 space-y-1">
											...
											<Select
												value={selectValue}
												onValueChange={(val) =>
													updateVar({ key: val === "__custom__" ? "" : val })
												}
												disabled={!allowEdit}
											>
												<SelectTrigger>
											...
											{isMultiline ? (
												...
												<div className="space-y-2">
													{(() => {
														...
														const valueSelect = currentValue
															? isPreset
																? currentValue
																: "__custom__"
															: "none";
															...
															<>
																...
																{valueSelect === "__custom__" && (
																	<Input
																		...
																		value={v.value}
																		onChange={(e) =>
																			updateVar({ value: e.target.value })
																		}
																		className="font-mono text-xs"
																		disabled={!allowEdit}
																	/>
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={!allowEdit || mutation.isPending || !name.trim()}
					>
						{mutation.isPending && (
			...
			<AlertDialog open={helpOpen} onOpenChange={setHelpOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							These are the most useful Netlab defaults you can override via
							environment variables (plus Skyforge’s multiline
