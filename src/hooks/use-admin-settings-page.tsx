...
function useAdminSettingsForwardSection(args: {
	...
	return useMemo(
		() => ({
			...
			saveQuickDeployRepoPending: auth.saveQuickDeployRepo.isPending,
			hasQuickDeployTemplateRows: auth.hasQuickDeployTemplateRows,
			quickDeployRepoDraft: auth.quickDeployRepoDraft,
			onSelectedQuickDeployOptionChange: auth.setSelectedQuickDeployOption,
			onQuickDeployRepoDraftChange: auth.setQuickDeployRepoDraft,
			onSaveQuickDeployRepo: () => auth.saveQuickDeployRepo.mutate(),
