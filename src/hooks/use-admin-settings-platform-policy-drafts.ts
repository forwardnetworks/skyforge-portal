...
export function useAdminSettingsPlatformPolicyDrafts({
  ...
  const [adminForwardTenantResetMode, setAdminForwardTenantResetMode] = useState<
    "hard-reset" | "curated-reset"
  >("curated-reset");
  ...
  const platformPolicyQ = useQuery({
    ...
    enabled: isAdmin && platformPolicyTargetUser.trim().length > 0,
    staleTime: 15_000,
    retry: false,
  });
  const adminForwardTenantResetRunsQ = useQuery<AdminForwardTenantResetRunsResponse>({
  ...
  const savePlatformProfiles = useMutation({
    ...
    onSuccess: async (policy) => {
      ...
      syncPlatformPolicyDrafts(policy, setPlatformProfileDraft, setPlatformQuotaDraft);
  ...
  const savePlatformQuota = useMutation({
    mutationFn: async () => {
      ...
      return putAdminPlatformUserQuota(platformPolicyTargetUser, {
        maxConcurrentLabs: parseNumericDraft(platformQuotaDraft.maxConcurrentLabs),
        maxPersistentLabs: parseNumericDraft(platformQuotaDraft.maxPersistentLabs),
        maxPersistentHours: parseNumericDraft(platformQuotaDraft.maxPersistentHours),
  ...
  const platformQuotaValidationErrors = useMemo(() => {
    ...
    return {
      maxConcurrentLabs:
        platformQuotaDraft.maxConcurrentLabs && !pattern.test(platformQuotaDraft.maxConcurrentLabs)
          ? "Use whole numbers only"
          : "",
      maxPersistentLabs:
        platformQuotaDraft.maxPersistentLabs && !pattern.test(platformQuotaDraft.maxPersistentLabs)
          ? "Use whole numbers only"
          : "",
      maxPersistentHours:
        platformQuotaDraft.maxPersistentHours && !pattern.test(platformQuotaDraft.maxPersistentHours)
          ? "Use whole numbers only"
          : "",
    };
  }, [platformQuotaDraft]);
  ...
  const platformQuotaHasErrors = useMemo(
    () => Object.values(platformQuotaValidationErrors).some((value) => Boolean(value)),
...
const profileCapabilityHints: Record<string, string[]> = {
  ...
  admin: ["manage_users", "manage_platform_operations", "reset_curated_forward_tenant"],
};
